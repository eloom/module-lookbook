...
namespace Eloom\Lookbook\Block\Adminhtml\LookbookCategory;
...
class AbstractCategory extends \Magento\Backend\Block\Template {
	...
	public function __construct(
		...
		\Eloom\Lookbook\Model\ResourceModel\LookbookCategory\Tree $categoryTree,
	...
	public function getCategory() {
		return $this->_coreRegistry->registry('lookbook_lookbook_category');
	}
	...
	public function getRoot($parentNodeCategory = null, $recursionLevel = 3) {
		...
		if ($root === null) {
			...
			if ($storeId) {
				...
				$rootId = \Eloom\Lookbook\Model\LookbookCategory::TREE_ROOT_ID;
			}
			...
			if ($root && $rootId != \Eloom\Lookbook\Model\LookbookCategory::TREE_ROOT_ID) {
				$root->setIsVisible(true);
			} elseif ($root && $root->getId() == \Eloom\Lookbook\Model\LookbookCategory::TREE_ROOT_ID) {
	...
	public function getNode($parentNodeCategory, $recursionLevel = 2) {
		...
		if ($node && $nodeId != \Eloom\Lookbook\Model\LookbookCategory::TREE_ROOT_ID) {
			$node->setIsVisible(true);
		} elseif ($node && $node->getId() == \Eloom\Lookbook\Model\LookbookCategory::TREE_ROOT_ID) {
