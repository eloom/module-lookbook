...
namespace Eloom\Lookbook\Block\Widget;

class FeaturedLooks extends \Eloom\Lookbook\Block\Item\AbstractItem implements \Magento\Widget\Block\BlockInterface {
	...
	public function _getCollection() {
		if ($this->_lookCollection === null) {
			...
			$this->_lookCollection = $this->_objectManager->get(\Eloom\Lookbook\Model\LookbookItem::class)->getCollection()->addAttributeToSelect('*');
			if ($lookIds) {
			...
			if ($lookbookIds) {
				$this->_lookCollection->getSelect()->joinLeft(
					['ccl' => $this->_lookCollection->getTable('lookbook_item_group')],
