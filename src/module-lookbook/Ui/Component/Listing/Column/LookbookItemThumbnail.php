...
class LookbookItemThumbnail extends \Magento\Ui\Component\Listing\Columns\Column {
	...
	protected $_basePath = 'lookbook/item_element';

	protected $_collectionClass = 'Eloom\Lookbook\Model\ResourceModel\LookbookItem\Collection';
	...
	public function __construct(
		...
		$this->_imageHelper = $this->_objectManager->get('Eloom\Lookbook\Helper\Image');
		...
		if (isset($dataSource['data']['items'])) {
			...
			foreach ($dataSource['data']['items'] as & $item) {
				...
				if (!empty($data['image'])) {
					...
					$thumbnail = $this->_imageHelper->init('lookbook/placeholder_thumbnail.jpg')->resize(75, 75)->__toString();
					$original = $this->_imageHelper->init('lookbook/placeholder_thumbnail.jpg')->resize(500, 500)->__toString();
				}
