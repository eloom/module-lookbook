...
class LookbookData extends \Magento\Framework\Model\AbstractModel {
  ...
  public function exportLookbooksToCSV() {
    ...
    $file = $this->fixtureManager->getFixture('Eloom_Lookbookpro::fixtures/lookbooks.csv');
  ...
  public function exportLookbookItemsToCSV() {
    ...
    $file = $this->fixtureManager->getFixture('Eloom_Lookbookpro::fixtures/lookbook_items.csv');
    $this->csvReader->saveData($file, $data);
    $result['message'] = '<ul>' . implode('', $message) . '</ul>';
    ...
    $select = $connection->select()->from(['cig' => $connection->getTableName('eloom_lookbook_item_group')])->limit(10000);
    ...
    $file = $this->fixtureManager->getFixture('Eloom_Lookbookpro::fixtures/lookbook_item_groups.csv');
    $this->csvReader->saveData($file, $data);
  ...
  public function exportLookbookCatagoriesToCSV() {
    ...
    $file = $this->fixtureManager->getFixture('Eloom_Lookbookpro::fixtures/lookbook_categories.csv');
    $this->csvReader->saveData($file, $data);
    $result['message'] = '<ul>' . implode('', $message) . '</ul>';
    ...
    $i = 0;
    $connection = $collection->getConnection();
    $select = $connection->select()->from(['cig' => $connection->getTableName('eloom_lookbook_category_lookbook')])->limit(10000);
    $itemgroup = $connection->fetchAll($select);
    foreach ($itemgroup as $group) {
      if ($i === 0) {
        $data[0] = array_keys($group);
    ...
    $file = $this->fixtureManager->getFixture('Eloom_Lookbookpro::fixtures/category_lookbook.csv');
    $this->csvReader->saveData($file, $data);
  ...
  public function assignLookbookToCategory() {
    ...
    if (count($sqlData)) {
      $connection->insertMultiple($connection->getTableName('eloom_lookbook_category_lookbook'), $sqlData);
    }
  }
  ...
  public function assignItemToLookbook() {
    ...
    if (count($sqlData)) {
      $connection->insertMultiple($connection->getTableName('eloom_lookbook_item_group'), $sqlData);
    }
  }
}
