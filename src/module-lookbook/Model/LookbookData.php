...
class LookbookData extends \Magento\Framework\Model\AbstractModel {
	...
	public function exportLookbookItemsToCSV() {
		...
		$select = $connection->select()->from(['cig' => $connection->getTableName('eloomlookbook_item_group')])->limit(10000);
	...
	public function exportLookbookCatagoriesToCSV() {
		...
		$i = 0;
		$connection = $collection->getConnection();
		$select = $connection->select()->from(['cig' => $connection->getTableName('eloomlookbook_category_lookbook')])->limit(10000);
		$itemgroup = $connection->fetchAll($select);
		foreach ($itemgroup as $group) {
			if ($i === 0) {
				$data[0] = array_keys($group);
	...
	public function assignLookbookToCategory() {
		...
		if (count($sqlData)) {
			$connection->insertMultiple($connection->getTableName('eloomlookbook_category_lookbook'), $sqlData);
		}
	}
	...
	public function assignItemToLookbook() {
		...
		if (count($sqlData)) {
			$connection->insertMultiple($connection->getTableName('eloomlookbook_item_group'), $sqlData);
		}
	}
}
