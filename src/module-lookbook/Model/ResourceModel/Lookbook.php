<?php
namespace Eloom\Lookbookpro\Model\ResourceModel;

use Magento\Eav\Model\Entity\AbstractEntity;
use Magento\Eav\Model\Entity\Attribute\AbstractAttribute;

class Lookbook extends \Magento\Eav\Model\Entity\AbstractEntity {
  /**
   * Store manager
   *
  ...
  protected $_lookbookItemGroupTable;
  ...
  public function __construct(
    ...
    \Eloom\Lookbookpro\Model\LookbookFactory $modelFactory,
                                               $data = []
  ) {
  ...
  protected function _construct() {
    $this->_read = 'eloomlookbook_read';
    $this->_write = 'eloomlookbook_write';
  }

  public function getEntityType() {
    if (empty($this->_type)) {
      $this->setType(\Eloom\Lookbookpro\Model\Lookbook::ENTITY);
    }
    return parent::getEntityType();
  }

  protected function _getDefaultAttributes() {
    return ['sort_order', 'created_at', 'updated_at'];
  }
  ...
  protected function _getDefaultAttributeModel() {
    return 'Eloom\Lookbookpro\Model\LookbookAttribute';
  }
  ...
  /**
   * Get positions of associated to category lookbooks
   *
   * @param \Eloom\Lookbookpro\Model\Lookbook $lookbook
   * @return array
   */
  public function getItemsPosition($lookbook) {
    $select = $this->getConnection()->select()->from(
      $this->getLookbookItemGroupTable(),
      ['item_id', 'position']
    )->where(
      'lookbook_id = :lookbook_id'
    );
    $bind = ['lookbook_id' => (int)$lookbook->getId()];

    return $this->getConnection()->fetchPairs($select, $bind);
  }


  public function getLookbookItemGroupTable() {
    if (!$this->_lookbookItemGroupTable) {
      $this->_lookbookItemGroupTable = $this->getTable('eloom_lookbook_item_group');
    }
    return $this->_lookbookItemGroupTable;
  }

  protected function _saveLookbookItems($lookbook) {
    $lookbook->setIsChangedProductList(false);
    $id = $lookbook->getId();
    /**
     * new category-product relationships
     */
    $items = $lookbook->getPostedItems();
     ...
     */
    $oldItems = $lookbook->getItemsPosition();

    $insert = array_diff_key($items, $oldItems);
    $delete = array_diff_key($oldItems, $items);

    /**
     * Find product ids which are presented in both arrays
     * and saved before (check $oldItems array)
     */
    $update = array_intersect_key($items, $oldItems);
    $update = array_diff_assoc($update, $oldItems);
     ...
     */
    if (!empty($delete)) {
      $cond = ['item_id IN(?)' => array_keys($delete), 'lookbook_id=?' => $id];
      $connection->delete($this->getLookbookItemGroupTable(), $cond);
    }
    ...
    if (!empty($insert)) {
      $data = [];
      foreach ($insert as $itemId => $position) {
        $data[] = [
          'lookbook_id' => (int)$id,
          'item_id' => (int)$itemId,
      ...
      $connection->insertMultiple($this->getLookbookItemGroupTable(), $data);
    }
    ...
    if (!empty($update)) {
      $newPositions = [];
      foreach ($update as $itemId => $position) {
        $delta = $position - $oldItems[$itemId];
        if (!isset($newPositions[$delta])) {
          $newPositions[$delta] = [];
        }
        $newPositions[$delta][] = $itemId;
      }

      foreach ($newPositions as $delta => $itemIds) {
        $bind = ['position' => new \Zend_Db_Expr("position + ({$delta})")];
        $where = ['lookbook_id = ?' => (int)$id, 'item_id IN (?)' => $itemIds];
        $connection->update($this->getLookbookItemGroupTable(), $bind, $where);
      }
    }

    if (!empty($insert) || !empty($delete)) {
      $itemIds = array_unique(array_merge(array_keys($insert), array_keys($delete)));
      // $this->_eventManager->dispatch(
      // 'catalog_category_change_lookbooks',
      // ['category' => $lookbook, 'item_ids' => $itemIds]
      // );

      $lookbook->setChangedProductIds($itemIds);
    }

    if (!empty($insert) || !empty($update) || !empty($delete)) {
      $lookbook->setIsChangedProductList(true);

      /**
       * Setting affected lookbooks to category for third party engine index refresh
       */
      $itemIds = array_keys($insert + $delete + $update);
      $lookbook->setAffectedProductIds($itemIds);
    }
    return $this;
  }

  protected function _afterSave(\Magento\Framework\DataObject $object) {
    $this->_saveLookbookItems($object);
    $this->_saveLookbookCategories($object);
    return parent::_afterSave($object);
  }

  public function getLookbookCategoryTable() {
    if (!$this->_lookbookCategoryTable) {
      $this->_lookbookCategoryTable = $this->getTable('eloom_lookbook_category_lookbook');
    }
    return $this->_lookbookCategoryTable;
  }

  public function getCategoriesPosition($lookbook) {
    $select = $this->getConnection()->select()->from(
  ...
  protected function _saveLookbookCategories($lookbook) {
    $lookbook->setIsChangedCategoryList(false);
    $id = $lookbook->getId();
    /**
     * new category-lookbook relationships
     */
    $categoryIds = $lookbook->getData('category_ids');
     ...
     * old category-product relationships
     */
    $oldCategories = $lookbook->getCategoriesPosition();

    $categories = [];

    foreach ($categoryIds as $categoryId) {
      if (isset($oldCategories[$categoryId])) {
    ...
    $insert = array_diff_key($categories, $oldCategories);
    $delete = array_diff_key($oldCategories, $categories);

    /**
     * Find product ids which are presented in both arrays
     * and saved before (check $oldCategories array)
     */
    $update = array_intersect_key($categories, $oldCategories);
    // get all elements of array1 having key exiting in array 2

    $update = array_diff_assoc($update, $oldCategories);
    // compare 2 arrays and return different elements (both key and value)

    $connection = $this->getConnection();

    /**
     * Delete categories from lookbook
     */
    if (!empty($delete)) {
      $cond = ['category_id IN(?)' => array_keys($delete), 'lookbook_id=?' => $id];
    ...
    if (!empty($update)) {
      $newPositions = [];
      foreach ($update as $categoryId => $position) {
        $delta = $position - $oldCategories[$categoryId];
        if (!isset($newPositions[$delta])) {
          $newPositions[$delta] = [];
        }
        $newPositions[$delta][] = $categoryId;
      }

      foreach ($newPositions as $delta => $categoryIds) {
        $bind = ['position' => new \Zend_Db_Expr("position + ({$delta})")];
        $where = ['lookbook_id = ?' => (int)$id, 'category_id IN (?)' => $categoryIds];
    ...
    if (!empty($insert) || !empty($delete)) {
      ...
      $lookbook->setChangedCategoryIds($categoryIds);
    }

    if (!empty($insert) || !empty($update) || !empty($delete)) {
      $lookbook->setIsChangedCategoryList(true);

      /**
       * Setting affected lookbooks to category for third party engine index refresh
       */
      $categoryIds = array_keys($insert + $delete + $update);
      $lookbook->setAffectedCategoryIds($categoryIds);
    }
    return $this;
  }
