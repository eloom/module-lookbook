...
namespace Eloom\Lookbookpro\Model\LookbookCategory;

use Eloom\Lookbookpro\Model\ResourceModel\LookbookCategory\CollectionFactory as CollectionFactory;
...
class DataProvider extends \Magento\Ui\DataProvider\AbstractDataProvider {
	...
	protected $_entityTypeCode = 'eloomlookbook_category';
	...
	public function getData() {
		...
		$data = $this->dataPersistor->get('eloomlookbook_category');
		if (!empty($data)) {
			$item = $this->collection->getNewEmptyItem();
			$item->setData($data);
			$this->loadedData[$item->getId()] = $item->getData();
			$this->dataPersistor->clear('eloomlookbook_category');
		}
		return $this->loadedData;
	}

	public function getCurrentItem() {
		$item = $this->registry->registry('lookbookpro_eloomlookbook_category');
		if ($item) {
			return $item;
		}
	...
	protected function filterFields(array $rawData) {
		...
		foreach ($imagesType as $image) {
			if (isset($data[$image])) {
				...
				if (file_exists($uploader->getBaseImageAbsolutePath($imageName))) {
					$data[$image][0]['size'] = filesize($uploader->getBaseImageAbsolutePath($imageName));
				}
			}
		}
		return $data;
	...
	private function getImageUploader() {
		if ($this->imageUploader === null) {
			$this->imageUploader = \Magento\Framework\App\ObjectManager::getInstance()->get(
				'Eloom\Lookbookpro\CategoryImageUpload'
			);
		}
	...
	public function getAttributesMeta(Type $entityType) {
		...
		foreach ($attributes as $attribute) {
			...
			if ($isScopeStore) {
				...
				$meta[$code]['disabled'] = ($item->getExistsStoreValueFlag($code) != 1);
			}
		}
		$result = [];
